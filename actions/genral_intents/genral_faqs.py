            ...
            if intent == "greet":
                ...
                if greet_count > 3:
                    ...
                    send_and_store_disposition_details(
                        ...
                        disposition_id=customer_says_hello_only,
                        flag=TIMEOUT_FLAG,
                    )
                ...
                if greet_count > 0:
                    ...
                    return [
                        FollowupAction("user_confirmation_form"),
                        SlotSet(REQUESTED_SLOT, None),
                        SlotSet("current_slot", current_slot),
                        SlotSet("greet_count", greet_count + 1),
                    ] + return_values
                if (
                    tracker.active_form.get("name") is not None
                    and requested_slot == "availability_status"
                    and trail_count is None
                ):
...
class ActionBye(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        dispatcher.utter_template("utter_G_bye_", tracker)
...
class ActionCallLater(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        send_and_store_disposition_details(
            ...
            dispatcher=dispatcher,
            user_message=user_message,
            disposition_id=customer_informed_call_later,
            flag=TIMEOUT_FLAG,
        )
        return [FollowupAction("action_listen")]
        ...
        if no_response_count is None:
            ...
            if no_response_count >= 2:
                ...
                send_and_store_disposition_details(
                    ...
                    dispatcher=dispatcher,
                    user_message="no_message",
                    disposition_id=customer_no_response,
                    flag=TIMEOUT_FLAG,
                )
                return [FollowupAction("action_listen"), AllSlotsReset()]
            else:
        ...
        for idx in range(0, len(bot_utter)):
            if bot_utter[idx]["data"]["buttons"]:
                dispatcher.utter_button_message(
                    bot_utter[idx]["text"], bot_utter[idx]["data"]["buttons"]
                )
...
class ActionHumanHandOff(Action):
    ...
    def run(self, dispatcher, tracker, domain):
        ...
        if human_handoff_count == 0:
            ...
            return get_return_values(tracker) + [
                SlotSet("human_handoff_count", human_handoff_count + 1)
            ]
        if human_handoff_count != 0:
            dispatcher.utter_template("utter_agent_will_connect", tracker)
            user_message = tracker.latest_message.get("text")
            send_and_store_disposition_details(
                ...
                dispatcher=dispatcher,
                user_message=user_message,
                disposition_id=customer_asked_human_handoff,
                flag=TIMEOUT_FLAG,
            )
            return [FollowupAction("action_listen"), AllSlotsReset()]
        return get_return_values(tracker)
