...
def write_to_database(response):
    ...
    try:
        ...
        ShriramHindiEvents.insert(sender_id=sender_id, user_id=user_id, request_id=request_id,
...
def store_call_logs(response):
    ...
    interested = get_interested_data(response)
    reason_data = get_reason_data(response)
    callback_time = get_callback_time_data(response)
    right_party_contact = get_right_party_contact_data(response)
    visit_status = get_visit_status_data(response)
    visit_type = get_visit_type_data(response)
    visit_date = get_visit_date_data(response)
    kyc_status = get_kyc_status_data(response)
