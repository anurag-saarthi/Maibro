...
URLS = [
    'http://20.204.237.17:8000/predict',
]
...
def webhook():
    ...
    if str(msg).lower() == "hello" or str(msg).lower() == "hey":
        ...
        if "<nlu_data>" not in msg:
            ...
            intent_response = [intent_response[0]["response"]]
            print("The value of msg",intent_response)

            msg = replace_english_words_in_english_utterance(str(msg))
            msg = msg.lower()
            try:
                entity_response = requests.post('http://20.204.204.228:8000/predict',json={"text": str(msg),"lang":"tamil"}).json()
            except:
